...
public class PoiView extends ViewPart implements Observer {
	...
	private IDialogSettings			_state					= Activator
																	.getDefault()
																	.getDialogSettingsSection("PoiView");	//$NON-NLS-1$
		...
		public Image getImage(final Object obj) {
			...
			if (obj instanceof PointOfInterest) {
				...
				if (poiCategory.equals("highway")) { //$NON-NLS-1$
					img = imageRegistry.get(Activator.IMG_KEY_CAR);
				} else if (poiCategory.equals("place")) { //$NON-NLS-1$
					img = imageRegistry.get(Activator.IMG_KEY_HOUSE);
				} else if (poiCategory.equals("waterway")) { //$NON-NLS-1$
					img = imageRegistry.get(Activator.IMG_KEY_ANCHOR);
				} else if (poiCategory.equals("amenity")) { //$NON-NLS-1$
					img = imageRegistry.get(Activator.IMG_KEY_CART);
				} else if (poiCategory.equals("leisure")) { //$NON-NLS-1$
					img = imageRegistry.get(Activator.IMG_KEY_STAR);
				} else if (poiCategory.equals("sport")) { //$NON-NLS-1$
					img = imageRegistry.get(Activator.IMG_KEY_SOCCER);
				} else {
					img = imageRegistry.get(Activator.IMG_KEY_FLAG);
				}
