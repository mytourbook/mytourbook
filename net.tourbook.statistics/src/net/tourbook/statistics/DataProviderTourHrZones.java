		...
		try {
			...
			while (result.next()) {
				...
				if (tourId == lastTourId) {
					 ...
					 * HR zones
					 */
					final int numberOfHrZones = result.getInt(15);
					final int[] hrZones = new int[numberOfHrZones];

					if (numberOfHrZones > 0) {
						hrZones[0] = result.getInt(16);
					}
					if (numberOfHrZones > 1) {
						hrZones[1] = result.getInt(17);
					}
					if (numberOfHrZones > 2) {
						hrZones[2] = result.getInt(18);
					}
					if (numberOfHrZones > 3) {
						hrZones[3] = result.getInt(19);
					}
					if (numberOfHrZones > 4) {
						hrZones[4] = result.getInt(20);
					}
					if (numberOfHrZones > 5) {
						hrZones[5] = result.getInt(21);
					}
					if (numberOfHrZones > 6) {
						hrZones[6] = result.getInt(22);
					}
					if (numberOfHrZones > 7) {
						hrZones[7] = result.getInt(23);
					}
					if (numberOfHrZones > 8) {
						hrZones[8] = result.getInt(24);
					}
					if (numberOfHrZones > 9) {
			...
			_tourHrZoneData.numberOfDaysInYear = _yearDays;
			_tourHrZoneData.years = _years;

			_tourHrZoneData.tourYearValues = ArrayListToArray.toInt(dbTourYear);
			_tourHrZoneData.tourMonthValues = ArrayListToArray.toInt(dbTourMonths);
			_tourHrZoneData.tourDOYValues = ArrayListToArray.toInt(dbAllYearsDOY);
			_tourHrZoneData.weekValues = ArrayListToArray.toInt(dbTourStartWeek);

			_tourHrZoneData.tourTimeStartValues = ArrayListToArray.toInt(dbTourStartTime);
			_tourHrZoneData.tourTimeEndValues = ArrayListToArray.toInt(dbTourEndTime);

			_tourHrZoneData.tourDistanceValues = ArrayListToArray.toInt(dbDistance);
			_tourHrZoneData.tourAltitudeValues = ArrayListToArray.toInt(dbAltitude);

			_tourHrZoneData.tourRecordingTimeValues = dbTourRecordingTime;
			_tourHrZoneData.tourDrivingTimeValues = dbTourDrivingTime;
