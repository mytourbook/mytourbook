...
/**
 * Instances of this class implement a Composite that positions and sizes children and allows
 * programmatic control of layout and border parameters. ViewForm is used in the workbench to lay
 * out a view's label/menu/toolbar local bar.
 * <p>
...
public class ViewForm extends Composite {
	 ...
	 * left and right edges of the form. The default value is 0.
	 */
	public int			marginWidth				= 0;
	/**
	 * marginHeight specifies the number of pixels of vertical margin that will be placed along the
	 * top and bottom edges of the form. The default value is 0.
	 */
	public int			marginHeight			= 0;
	/**
	 * horizontalSpacing specifies the number of pixels between the right edge of one cell and the
	 * left edge of its neighbouring cell to the right. The default value is 1.
	 */
	public int			horizontalSpacing		= 1;
	/**
	 * verticalSpacing specifies the number of pixels between the bottom edge of one cell and the
	 * top edge of its neighbouring cell underneath. The default value is 1.
	 */
	public int			verticalSpacing			= 1;

	/**
	 * Color of innermost line of drop shadow border. NOTE This field is badly named and can not be
	 * fixed for backwards compatibility. It should be capitalized.
	 * 
	 * @deprecated
	 */
	@Deprecated
	...
	/**
	 * Color of middle line of drop shadow border. NOTE This field is badly named and can not be
	 * fixed for backwards compatibility. It should be capitalized.
	 * 
	 * @deprecated
	 */
	...
	/**
	 * Color of outermost line of drop shadow border. NOTE This field is badly named and can not be
	 * fixed for backwards compatibility. It should be capitalized.
	 * 
	 * @deprecated
	 */
	...
	public ViewForm(final Composite parent, final int style) {
		...
		final int[] events = new int[] { SWT.Dispose, SWT.Paint, SWT.Resize };
	...
	static int checkStyle(final int style) {
		final int mask = SWT.FLAT | SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT;
...
@Override
	public Rectangle computeTrim(final int x, final int y, final int width, final int height) {
		checkWidget();
		final int trimX = x - borderLeft - highlight;
		final int trimY = y - borderTop - highlight;
		final int trimWidth = width + borderLeft + borderRight + 2 * highlight;
		final int trimHeight = height + borderTop + borderBottom + 2 * highlight;
	...
	/**
	 * Returns the Control that appears in the top left corner of the pane. Typically this is a
	 * label such as CLabel.
	 * 
	...
	/**
	 * Returns the control in the top right corner of the pane. Typically this is a Close button or
	 * a composite with a Menu and Close button.
	 * 
	...
	void onPaint(final GC gc) {
		...
		final Color border = getDisplay().getSystemColor(BORDER1_COLOR);
		if (showBorder) {
			...
			if (highlight > 0) {
				...
				final int[] shape = new int[] {
						...
						x1,
						y1 + highlight };
				final Color highlightColor = getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION);
				gc.setBackground(highlightColor);
				gc.fillPolygon(shape);
		}
		}
		if (separator > -1) {
// don't draw this line in a flat design
//			gc.setForeground(border);
//			gc.drawLine(borderLeft + highlight, separator, size.x - borderLeft - borderRight - highlight, separator);
	...
	/**
	 * Sets the content. Setting the content to null will remove it from the pane - however, the
	 * creator of the content must dispose of the content.
	 * 
	 * @param content
	...
	/**
	 * Set the control that appears in the top center of the pane. Typically this is a toolbar. The
	 * topCenter is optional. Setting the topCenter to null will remove it from the pane - however,
	 * the creator of the topCenter must dispose of the topCenter.
	 * 
	 * @param topCenter
	...
	/**
	 * Set the control that appears in the top left corner of the pane. Typically this is a label
	 * such as CLabel. The topLeft is optional. Setting the top left control to null will remove it
	 * from the pane - however, the creator of the control must dispose of the control.
	 * 
	 * @param c
