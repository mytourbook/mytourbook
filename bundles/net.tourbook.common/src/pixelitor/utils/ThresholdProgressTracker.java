...
public abstract class ThresholdProgressTracker implements ProgressTracker {
   ...
   protected ThresholdProgressTracker(final String opName, final int numTotalUnits) {
   ...
   @Override
   public void unitsDone(final int units) {
      assert units > 0;
   ...
   private void update() {
      ...
      if (!isTrackingVisible) {
         final double elapsedTime = System.currentTimeMillis() - startTimeMillis;
      ...
      if (isTrackingVisible) {
         final int currentPercent = (int) (completedUnits * 100.0 / numTotalUnits);
