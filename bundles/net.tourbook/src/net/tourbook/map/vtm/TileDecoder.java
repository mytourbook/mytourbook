...
public class TileDecoder extends PbfDecoder {
	...
	{
		mFeaturePool = new Pool<Feature>() {

			int count;

			@Override
			protected boolean clearItem(final Feature item) {
	...
	static class Feature extends Inlist<Feature> {
		...
		boolean match(final short otherTags[], final int otherNumTags, final int otherType) {
	...
	public TileDecoder(final String locale) {
		mLocale = locale;
	}

	@Override
	public boolean decode(final Tile tile, final ITileDataSink mapDataCallback, final InputStream is)
			throws IOException {
	...
	private int decodeCoordinates(final int type, final Feature feature) throws IOException {

		final int bytes = decodeVarint32();
		fillBuffer(bytes);
		...
		final MapElement elem = feature.elem;
		...
		// test bbox for outer..
		final boolean isOuter = true;
		final boolean simplify = false; //mTile.zoomLevel < 14;
		final int pixel = simplify ? 7 : 3;

		//int xmin = Integer.MAX_VALUE, xmax = Integer.MIN_VALUE;
		//int ymin = Integer.MAX_VALUE, ymax = Integer.MIN_VALUE;

		for (final int end = bufferPos + bytes; bufferPos < end;) {
...
//                continue;
			...
			final int dx = (curX - prevX);
			final int dy = (curY - prevY);
	...
	private void decodeFeature(final ArrayList<Feature> features) throws IOException {
		final int bytes = decodeVarint32();
		final int end = position() + bytes;
	...
	private boolean decodeLayer() throws IOException {
		...
		final Tag layerTag = new Tag("layer", name);
		if (debug) {
		...
		for (Feature f : features) {
			...
			for (int j = 0; j < (f.numTags << 1); j += 2) {
				...
				String key;
				final String val = values.get(f.tags[j + 1]);
