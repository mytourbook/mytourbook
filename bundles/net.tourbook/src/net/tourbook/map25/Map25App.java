...
public class Map25App extends GdxMap implements OnItemGestureListener, ItemizedLayer.OnItemGestureListener<MarkerInterface> {
   ...
   private float                   _currentCartography_Luminance;
   //
...
//   private OpenGLTestLayer         _layer_OpenGLTest;
   ...
   private static class RuleVisitor_Luminence extends RuleVisitor {


//      private int   __counter;
      private float __cartographyLuminance;

      public RuleVisitor_Luminence(final float cartographyLuminance) {

         __cartographyLuminance = cartographyLuminance;
      }

      @Override
      public void apply(final Rule rule) {

//         final boolean isRuleLogged = false;
//
//         final String ruleLog = ""
...
//            boolean isLogStyle = false;

            if (style instanceof AreaStyle) {

//               isLogStyle = true;
//
//               final AreaStyle areaStyle = (AreaStyle) (style);
//               name = String.format("%-20s", areaStyle.style);
//               color = String.format("%08X", areaStyle.color & 0xFFFF_FFFF);
//               hash = Integer.toString(areaStyle.hashCode());

               setFieldValue(style, "color", __cartographyLuminance);
...
//               name = String.format("%-20s", lineStyle.style);
//               color = String.format("%08X", lineStyle.color & 0xFFFF_FFFF);
//               hash = Integer.toString(lineStyle.hashCode());

               setFieldValue(style, "color", __cartographyLuminance);
               setFieldValue(style, "stippleColor", __cartographyLuminance);
            }

//            if (isLogStyle) {
//
//               if (isRuleLogged == false) {
//                  isRuleLogged = true;
...
//            }
      ...
      private int getAdjustedColor(final int fillColor, final float luminance) {
         ...
         if (luminance >= 0) {
            ...
            red = (int) (red * luminance);
            green = (int) (green * luminance);
            blue = (int) (blue * luminance);
      ...
      private void setFieldValue(final RenderStyle<?> style, final String fieldName, final float colorBrightness) {
         ...
         final int styleHashCode = style.hashCode();

         try {

            final Field colorField = style.getClass().getField(fieldName);

            final int colorValue = colorField.getInt(style);
            final int colorValue_Adjusted = getAdjustedColor(colorValue, colorBrightness);

            colorField.setAccessible(true);
            colorField.setInt(style, colorValue_Adjusted);
...
// SET_FORMATTING_ON
   ...
   public void setLayer_Cartography_Options(final float luminance) {
...
//               && mapProvider.vtmTheme != VtmThemes.OPENMAPTILES
   ...
   private void updateLayer_Cartography() {
      ...
      // check if cartography has changed
      if (_cartography_Luminance == _currentCartography_Luminance) {
      ...
      // keep state to compare it the next time
      _currentCartography_Luminance = _cartography_Luminance;
      ...
      if (mapTheme instanceof RenderTheme) {
         ...
         renderTheme.traverseRules(new RuleVisitor_Luminence(_cartography_Luminance));
