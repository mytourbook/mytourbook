...
public class ClusterMarkerRenderer extends MarkerRenderer {
     ...
     */
    public ClusterMarkerRenderer(final MarkerLayer markerLayer, final MarkerSymbol defaultSymbol, final ClusterMarkerRenderer.ClusterStyle style) {
    ...
    protected Bitmap getClusterBitmap(int size) {
        ...
        final ClusterUtils.ClusterDrawable drawable = new ClusterUtils.ClusterDrawable(
     ...
     */
    private void repopulateCluster(final int size, final double scale) {
        ...
        final Clustered[] tmp = new Clustered[size];

        // clear grid map to count items that share the same "grid slot"
        mGridMap.clear();

        for (int i = 0; i < size; i++) {
            final Clustered it = tmp[i] = new Clustered();
    ...
    public synchronized void update(final GLViewport v) {
        ...
        for (final InternalItem itm : mItems) {
            final Clustered it = (Clustered) itm;
        ...
        for (final InternalItem itm : mItems) {
            ...
            if (it.clusterSize > 0) {
                ...
                final Bitmap bitmap = getClusterBitmap(it.clusterSize + 1);
