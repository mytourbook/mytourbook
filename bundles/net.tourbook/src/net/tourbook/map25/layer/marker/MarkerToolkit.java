...
public class MarkerToolkit {
   ...
   public MarkerSymbol createAdvanceSymbol(final MarkerItem mItem, final Bitmap poiBitmap) {
      ...
      final int titleWidth = ((int) textPainter.getTextWidth(mItem.title) + 2 * margin);
      final int titleHeight = ((int) textPainter.getTextHeight(mItem.title) + 2 * margin);
      ...
      final int ySize = titleHeight + symbolWidth + dist2symbol;

      // markerCanvas, the drawing area for all: title, description and symbol
      final Bitmap markerBitmap = CanvasAdapter.newBitmap(xSize, ySize, 0);
      ...
      final Bitmap titleBitmap = CanvasAdapter.newBitmap(titleWidth + margin, titleHeight + margin, 0);
   ...
   public Bitmap createPoiBitmap(final MarkerShape shape) {
      ...
      final float half = _symbolSizeInt / 2;
