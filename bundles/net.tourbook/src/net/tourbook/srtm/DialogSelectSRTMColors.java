...
//               defaultSize.x = shellSize.x;
   ...
   protected final void createButtonsForButtonBar(final Composite parent) {
      ...
      button.addSelectionListener(widgetSelectedAdapter(selectionEvent -> onVertex_Add()));
      ...
      button.addSelectionListener(widgetSelectedAdapter(selectionEvent -> onVertex_AddMultiple()));
      ...
      _btnRemove = createButton(
            ...
            false);
      _btnRemove.addSelectionListener(widgetSelectedAdapter(selectionEvent -> onVertex_Remove()));
      ...
      button = createButton(
            ...
            false);
      button.addSelectionListener(widgetSelectedAdapter(selectionEvent -> sortVertexsAndUpdateProfile()));
      ...
      _btnApply = createButton(
            ...
            false);
      _btnApply.addSelectionListener(widgetSelectedAdapter(selectionEvent -> onApply()));
...
//      colorContainer.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_RED));
   ...
   private void createUI_70_VertexFieds() {
      ...
      final ArrayList<RGBVertex> allRGBVerticies = getVertexImage().getRgbVertices();

      final int numVerticies = allRGBVerticies.size();
      if (numVerticies == 0) {
         return;
      }
       ...
       * Field listener
       */
      final SelectionListener delete_SelectionListener = widgetSelectedAdapter(selectionEvent -> {
         onField_RemoveCheckbox(numVerticies);
      });

      final SelectionListener elevation_SelectionListener = widgetSelectedAdapter(selectionEvent -> {
         onField_SelectElevation(selectionEvent.widget);
      });

      final MouseWheelListener elevation_MouseWheelListener = mouseEvent -> {
         UI.adjustSpinnerValueOnMouseScroll(mouseEvent);
         onField_SelectElevation(mouseEvent.widget);
      };

      final MouseListener color_MouseListener = mouseDownAdapter(mouseEvent -> {
         onField_MouseDown(mouseEvent);
      });
       ...
       * fields
       */
      _lblColor = new Label[numVerticies];
      _spinnerElevation = new Spinner[numVerticies];
      _chkDelete = new Button[numVerticies];

      _isUIUpdated = true;
      {
         for (int vertexIndex = numVerticies - 1; vertexIndex >= 0; vertexIndex--) {

            final RGBVertex vertex = allRGBVerticies.get(vertexIndex);
            final RGB vertexRGB = vertex.getRGB();
            final Color bgColor = new Color(vertexRGB);

            {
               ...
               spinnerElevation.addSelectionListener(elevation_SelectionListener);
               spinnerElevation.addMouseWheelListener(elevation_MouseWheelListener);
            }
            {
               /*
                * Label: Elevation color
                */
               final Label lblColor = _lblColor[vertexIndex] = new Label(vertexContainer, SWT.CENTER | SWT.BORDER | SWT.SHADOW_NONE);
               lblColor.setLayoutData(gdColor);
               lblColor.setBackground(bgColor);
               lblColor.addMouseListener(color_MouseListener);
            ...
            }
            {
               ...
               checkbox.addSelectionListener(delete_SelectionListener);
            }
         }
      }
      _isUIUpdated = false;
       ...
       */
      if (numVerticies <= 2) {
         for (int ix = 0; ix < numVerticies; ix++) {
      ...
      // update color async otherwise the dark theme would overwrite it
      parent.getDisplay().asyncExec(() -> {
         ...
         for (int vertexIndex = 0; vertexIndex < allRGBVerticies.size(); vertexIndex++) {

            final RGBVertex vertex = allRGBVerticies.get(vertexIndex);
            final RGB vertexRGB = vertex.getRGB();
            final Color vertexColor = new Color(vertexRGB);

            _lblColor[vertexIndex].setBackground(vertexColor);
         }
      });
   }
...
//   }
   ...
   private void onField_MouseDown(final MouseEvent e) {
      ...
      if (e.button == 3) {
         ...
         final Color labelColor = new Color(rgb);
   ...
   private void onField_RemoveCheckbox(final int vertexSize) {
   ...
   private void onField_SelectElevation(final Widget widget) {
   ...
   private void onVertex_AddMultiple() {
      // ensure the field list is updated and not unsorted
      sortVertexsAndUpdateProfile();
...
//               elevFields[elevFields.length - 1].setFocus();
   ...
   private void onVertex_Remove() {

      final ProfileImage rgbVertexImage = getVertexImage();
      final ArrayList<Integer> vertexRemoveIndex = new ArrayList<>();
...
//      final ArrayList<RGBVertex> rgbVerticies = _dialogProfile.getRgbVertexImage().getRgbVerticies();
      ...
      final ArrayList<RGBVertex> newRgbVerticies = new ArrayList<>();
