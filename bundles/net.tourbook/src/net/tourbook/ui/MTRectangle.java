...
public final class MTRectangle implements Serializable {
	 ...
	 *            the height of the rectangle
	 */
	public MTRectangle(final int x, final int y, final int width, final int height) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
	...
	public void add(final int xx, final int yy) {

		final int left = x < xx ? x : xx;
		final int top = y < yy ? y : yy;
	...
	public void add(final int xx, final int yy, final int ww, final int hh) {
	 ...
	 *                </ul>
	 */
	public void add(final MTRectangle rect) {
		if (rect == null) {
	...
	public MTRectangle union(final MTRectangle rect) {
		...
		rhs = rect.y + rect.height;
		final int bottom = lhs > rhs ? lhs : rhs;
		return new MTRectangle(left, top, right - left, bottom - top);
	}
