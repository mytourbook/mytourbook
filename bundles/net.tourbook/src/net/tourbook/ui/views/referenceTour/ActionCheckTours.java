...
final class ActionCheckTours extends Action {
	...
	private final ElevationCompareResultView	fCompareResultView;

	ActionCheckTours(final ElevationCompareResultView compareResultView) {
	...
	public void run() {
		...
		if (selection.size() > 0) {
			...
			for (final Object tour : selection.toArray()) {
				...
				if (tour instanceof TVIElevationCompareResult_ComparedTour) {
					final TVIElevationCompareResult_ComparedTour comparedTour = (TVIElevationCompareResult_ComparedTour) tour;
