...
/*
 * found here: http://svn.openstreetmap.org/applications/viewer/jmapviewer/src/org/openstreetmap/gui/jmapviewer/OsmMercator.java
 */
...
public class OsmMercator {
	...
	public static int falseEasting(final int aZoomlevel) {
		return getMaxPixels(aZoomlevel) / 2;
	}

	public static int falseNorthing(final int aZoomlevel) {
	 ...
	 * @return
	 */
	public static int getMaxPixels(final int aZoomlevel) {
	 ...
	 */
	public static int LatToY(double aLat, final int aZoomlevel) {
		if (aLat < MIN_LAT)
			aLat = MIN_LAT;
		else if (aLat > MAX_LAT)
			aLat = MAX_LAT;
		final double sinLat = Math.sin(Math.toRadians(aLat));
	 ...
	 * @author Jan Peter Stotz
	 */
	public static int LonToX(final double aLongitude, final int aZoomlevel) {
		final int mp = getMaxPixels(aZoomlevel);
	...
	public static double radius(final int aZoomlevel) {
	...
	public static double YToLat(int aY, final int aZoomlevel) {
		...
		final double latitude = (Math.PI / 2) - (2 * Math.atan(Math.exp(-1.0 * aY / radius(aZoomlevel))));
