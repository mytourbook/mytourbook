...
public class PrefPageAppearanceTourChart extends PreferencePage implements IWorkbenchPreferencePage {
	...
	private Control createUI10TabGraphs(final Composite parent) {
		...
		GridLayoutFactory.swtDefaults().applyTo(container);
		{
			...
			createUI16GraphOptions(container);
		}
...
//		table.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_GREEN));
	...
	private void createUI16GraphOptions(final Composite parent) {

		// group: default graphs
		final Group group = new Group(parent, SWT.NONE);
		group.setText(Messages.Pref_Graphs_Group_GraphOptions);
		GridDataFactory.fillDefaults().grab(true, false).applyTo(group);
		GridLayoutFactory.swtDefaults().applyTo(group);
		{
			/*
			...
			/*
			 * checkbox: skipped values
			 */
			_chkShowBreaktimeValues = new Button(group, SWT.CHECK);
			_chkShowBreaktimeValues.setText(Messages.Graph_Label_ShowBreaktimeValues);
			_chkShowBreaktimeValues.setToolTipText(Messages.Graph_Label_ShowBreaktimeValues_Tooltip);
		}
	}
	...
	protected void performDefaults() {
		...
		if (selectedTabIndex != -1) {
			...
			if (selectedTabItem.equals(_tabVisibleGraphs)) {
				...
				_chkShowBreaktimeValues.setSelection(//
						_prefStore.getDefaultBoolean(ITourbookPreferences.GRAPH_IS_BREAKTIME_VALUES_VISIBLE));
	...
	private void restoreGraphs() {
		...
		_chkShowBreaktimeValues.setSelection(//
				_prefStore.getBoolean(ITourbookPreferences.GRAPH_IS_BREAKTIME_VALUES_VISIBLE));
	}
	...
	private void saveGraphs() {
		...
		_prefStore.setValue(
				...
				_chkShowBreaktimeValues.getSelection());
	}
