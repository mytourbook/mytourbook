...
/**
 * Blob store which maps UUIDs to blobs on disk. The UUID is mapped to a file in the file-system and
 * the blob is the file contents. For scalability, the blobs are split among 255 directories with
 * the names 00 to FF.
 */
...
// SET_FORMATTING_OFF
	private static byte[]	randomArray	= {
		-43, 31, 52, 60, 111, -10, -30, 121, -127, -59, -112, -8, 92, -123, 28, -51, 107, 52, -55, 14, //
		70, 5, -23, -68, 44, 91, -91, -107, -79, 93, 17, 112, 4, 41, -26, 96, 116, 104, -81, -69, 3, //
		-109, -10, -86, 30, -54, -73, -14, 47, -2, -67, 25, -8, -63, 2, 119, -123, 125, 12, 76, -43, //
		-37, 79, 69, -123, -54, 80, -106, -66, -99, -66, 80, -66, -37, -106, -87, 117, 95, 10, 77, -42, //
		32, 5, -111, 29, 96, 61, 110, 106, 118, -101, -97, 28, -10, -13, 30, -58, -5, 81, 43, -83, 96, //
		-75, 99, -87, -39, 4, 47, 105, -52, -121, 67, 74, -112, -71, -115, 96, 34, -74, 90, 36, -39, -108, //
		-68, 107, 8, 1, 27, -40, 60, 35, -5, -62, 7, -100, -43, -25, 37, 85, -45, 29, -95, 27, 43, 90, 65, //
		55, 23, -21, 51, 104, 123, -118, 76, -108, 115, 119, 81, 54, 39, 46, -107, -65, 79, 16, -34, -61, //
		-39, -102, 118, -85, -100, 71, 58, -9, 127, 56, 118, -124, 79, 77, 92, -96, -21, -41, -69, 23, 57, //
		11, -81, -68, -111, 50, 56, -21, -17, -86, -118, 17, -45, 56, 98, 101, 126, 27, 57, -45, -112, -50, //
		-49, -77, 111, -96, 50, -13, 69, 42, -68, -98, 121, -1, -84, 9, 31, 59, 115, -63, 28, 55, 101, //
		9, 117, -45, 112, 61, 31, -33, -49, -107, -11, 115, -13, 69, -37, -120, -108, -75, 77, -6, 101, //
		-33, -116, -62, -115, 44, -73 //
		};
//  SET_FORMATTING_ON

	/**
	 * The limit is the maximum number of directories managed by this store. This number must be
	 * power of 2 and do not exceed 256. The location should be an existing valid directory.
	 */

	public BlobStore(final IFileStore store, final int limit) {
...
//	public UniversalUniqueIdentifier addBlob(final IFileStore target, final boolean moveContents) throws CoreException {
//		final UniversalUniqueIdentifier uuid = new UniversalUniqueIdentifier();
//		folderFor(uuid).mkdir(EFS.NONE, null);
//		final IFileStore destination = fileFor(uuid);
//		if (moveContents) {
...
//	}
	 ...
	 */
	private void appendByteString(final StringBuffer buffer, final byte value) {
		String hexString;
		if (value < 0) {
	...
	/*
	 * (non-Javadoc) Converts an array of bytes into a String.
	 * @see UniversalUniqueIdentifier#toString()
	 */
	private String bytesToHexString(final byte[] b) {
	 ...
	 * Deletes a blobFile.
	 */
	public void deleteBlob(final String uuid) {
	 ...
	 */
	public void deleteBlobs(final Set<String> set) {
		for (final String id : set) {
			deleteBlob(id);
		}
	}

	public IFileStore fileFor(final String uuid) {
		final IFileStore root = folderFor(uuid);
		return root.getChild(bytesToHexString(uuid.getBytes()));
	}

	/**
	 * Find out the name of the directory that fits better to this UUID.
	 */
	public IFileStore folderFor(final String uuid) {
		byte hash = hashUUIDbytes(uuid);
		hash &= mask; // limit the range of the directory
		final String dirName = Integer.toHexString(hash + (128 & mask)); // +(128 & mask) makes sure 00h is the lower value
		return localStore.getChild(dirName);
	}

	public InputStream getBlob(final String uuid) throws CoreException {
		final IFileStore blobFile = fileFor(uuid);
		return blobFile.openInputStream(EFS.NONE, null);
	}

	/**
	 * Converts a byte array into a byte hash representation. It is used to get a directory name.
	 */
	protected byte hashUUIDbytes(final String uuid) {
		final byte[] bytes = uuid.getBytes();
		byte hash = 0;
