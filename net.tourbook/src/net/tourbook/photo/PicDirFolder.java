...
//				_picDirImages.showImages(_selectedFolder);
	 ...
	 */
	void selectFolder(	final String selectedFolderName,
						final boolean isMoveUpHierarchyWhenFolderIsInvalid,
						final boolean isNavigation) {

		_isNavigation = isNavigation;

		boolean isSelectedFolderAvailable = false;
		File selectedFolder = null;

		if (selectedFolderName != null) {

			final File folderFile = new File(selectedFolderName);
			if (folderFile.isDirectory()) {
				selectedFolder = folderFile;
				isSelectedFolderAvailable = true;
			}
			...
			if (selectedFolder == null && isMoveUpHierarchyWhenFolderIsInvalid) {
				...
				IPath folderPath = new Path(selectedFolderName);
		...
		if (selectedFolderName != null && isSelectedFolderAvailable == false) {
			...
			MessageDialog.openInformation(
					...
					NLS.bind(Messages.Pic_Dir_Dialog_FolderIsNotAvailable_Message, selectedFolderName));
		}
		...
		// select folder which triggers onSelectFolder(...)
		_folderViewer.setSelection(treeSelection, true);
	}
