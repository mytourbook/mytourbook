...
/**
 * Manages icons for the application. This is necessary as we could easily end up creating thousands
 * of icons bearing the same image.
 */
public class IconCacheOLD {

	// Stock images
	public final int		shellIcon			= 0, iconClosedDrive = 1, iconClosedFolder = 2, iconFile = 3,
			iconOpenDrive = 4, iconOpenFolder = 5, cmdCopy = 6, cmdCut = 7, cmdDelete = 8, cmdParent = 9,
			cmdPaste = 10, cmdPrint = 11, cmdRefresh = 12, cmdRename = 13, cmdSearch = 14;
	...
	private Image createStockImage(final Display display, final String path) {
		final InputStream stream = IconCacheOLD.class.getResourceAsStream(path);
	...
	public void freeResources() {
		...
		if (iconCache != null) {
			for (final Enumeration it = iconCache.elements(); it.hasMoreElements();) {
				final Image image = (Image) it.nextElement();
				image.dispose();
			}
		}
	...
	public void initResources(final Display display) {
		if (stockImages == null) {
			...
			for (int i = 0; i < stockImageLocations.length; ++i) {
				final Image image = createStockImage(display, stockImageLocations[i]);
				if (image == null) {
					freeResources();
