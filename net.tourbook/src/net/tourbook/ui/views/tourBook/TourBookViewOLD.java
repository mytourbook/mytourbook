...
import net.tourbook.tour.TourOLD;
...
public class TourBookViewOLD extends ViewPart implements ITourChartViewer {
...
//		viewTbm.add(fActionShowDetailTourChart);
	...
	private void createContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				TourBookViewOLD.this.fillContextMenu(manager);
			}
		});
	...
	private Control createDetail(Composite parent) {
		...
		fTour = new TourOLD(fPageBookDetailChart, SWT.FLAT);
		...
		fTourViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selectedItem instanceof TVITourBookYear) {
					...
					fStatistics.refreshStatistic(fActivePerson,
							fActiveTourTypeId,
							fStatisticYear,
							false);
...
// TableColumnFactory.DEVICE_NAME.createColumn(fColumnManager, pixelConverter);
	...
	void refreshTour(TourData tourData) {
		fTourChart.updateTourChart(fTourChartTourData, fTourChartConfig, false);
	...
	private void showTourChartInView(long tourId) {
		...
		if (fTourChartTourData != null) {
			...
			firePostSelection(new SelectionTourData(fTourChartTourData));
		}
	}
