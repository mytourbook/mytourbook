...
public class DialogExportTour extends TitleAreaDialog {
	...
	private String							_exportState_CourseName;
	...
	private boolean							_exportState_IsCourses;
...
//		GridDataFactory.fillDefaults().applyTo(container);
	...
	private void doExport() throws IOException {
		...
		if (_isGPX) {
			...
			_exportState_IsCourses = _rdoTcxCourses.getSelection();
			_exportState_CourseName = _comboTcxCourseName.getText();
		}
		...
		if (_tourDataList.size() == 1) {
			...
			doExport_10_Tour(tracks, wayPoints, tourMarkers, tourLap, exportFileName);
	...
	private void doExport_05_Runnable(final IProgressMonitor monitor, final String exportFileName) throws IOException {
		...
		if (_exportState_IsMergeTours) {
			...
			for (final TourData tourData : _tourDataList) {
				...
				doExport_10_Tour(tracks, wayPoints, tourMarkers, tourLap, exportFilePathName);
	...
	private void doExport_10_Tour(	final ArrayList<GarminTrack> tracks,
		...
		if (_isTCX) {
			...
			vc.put("iscourses", _exportState_IsCourses); //$NON-NLS-1$
			vc.put("coursename", _exportState_CourseName); //$NON-NLS-1$
		}
